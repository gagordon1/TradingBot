from quadInvesting.simulation import context, simulate
from quadInvesting.MagicDB import MagicDB
from quadInvesting.DatabaseAPI.IexDB import IexDB as iDB
from quadInvesting.DatabaseAPI.PolygonDB import PolygonDB as pDB
from quadInvesting.DataAnalysis import DataAnalysis as DA
...
def strat(context, symbol):
	...
	if type(price) != str:
		...
		if MA2 > MA1*1.003 and context.cash > price:
			context.buy_position(symbol, 1)

		elif MA1 > MA2*1.003 and context.positions[symbol] > 0:
