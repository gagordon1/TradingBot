...
def init(context, end_date, C1, C2):
	...
	for num in ["10", "20", "30", "40", "50", "70", "90", "100", "150", "200"]:
		val = int(num)
	...
	context.variables["Buy?"] = True
	context.variables["NN"] = load_NN('Algorithms/NeuralNets/NET6.pth')
...
def strat(context, C1, C2):
	...
	if isinstance(price, float):
		...
		vec = normalize(vec)
		vavgs = [volume]
		vavgs.extend([sum(context.variables["V{}".format(num)])/int(num)
		...
		# print(vec)
		buy_ = context.variables["Buy?"]
		if pred[2].item() > .9999999995 and buy_ == True:
			context.buy(C1,C2, context.positions[C2])
			context.variables["Buy?"] = price

		elif isinstance(buy_, float):
			# if pred[0].item() > .99995:
			# 	context.sell(C1,C2, context.positions[C1])
				# context.variables["Buy?"] = True
